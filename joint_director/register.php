...
}else if($firstnm=="" || $lastnm=="" || $email=="" || $pass=="" || $repass=="" || $regional_office==""){
  echo "<script>alert('Please Enter All Fields!!')</script>";
}else{

  $sql=mysqli_query($con,"select * from `joint_director` where `email`='$email'");
  $count=mysqli_num_rows($sql);
  if($count>0){
    echo "<script>alert('Email Id Already Registered With Us!!')</script>";
  }else{

    $sqmins=mysqli_query($con,"INSERT INTO `joint_director`(`firstnm`, `lastnm`, `email`, `pass`,`regional_office`) VALUES ('$firstnm','$lastnm','$email','$pass','$regional_office')");
