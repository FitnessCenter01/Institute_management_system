...
echo "<script>alert('password not match!!')</script>";
  ...
  $sql=mysqli_query($con,"select * from `director` where `email`='$email'");
  $count=mysqli_num_rows($sql);
  if($count>0){
    echo "<script>alert('Email Id Already Registered With Us!!')</script>";
  }else{

    $sqmins=mysqli_query($con,"INSERT INTO `director`(`firstnm`, `lastnm`, `email`, `pass`,`regional_office`) VALUES ('$firstnm','$lastnm','$email','$pass','$regional_office')");
